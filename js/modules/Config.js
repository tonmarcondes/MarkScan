...
export class Config {
  ...
  _saveConfig() {
    try {
      ...
      throw new Error('Não foi possível salvar as configurações neste navegador. Verifique o espaço e as permissões de armazenamento.');
    }
  }
   ...
   */
  update(updates) {
    const previous = JSON.parse(JSON.stringify(this.config));
    for (const [key, value] of Object.entries(updates)) {
      this.config[key] = value && typeof value === 'object'
        ? { ...this.config[key], ...value } : value;
    }
    try { this._saveConfig(); } catch (error) { this.config = previous; throw error; }
  }
