...
(async () => {
  ...
  try {
    ...
    assert.ok(await page.evaluate(()=>app.review.pending));
    ...
    await page.waitForFunction(() => document.querySelector("#import-status")?.textContent.includes("Não foi possível abrir"));
